...
contract AgenticIdTest is Test {
    ...
    function setUp() public {
        ...
        nft = new AgenticId("Graph Mini Apps", "GMINI", "ipfs://", address(verifier), address(this));
    ...
    function test_MintBindsEnsNameAndRecoversProver() public {
        (uint256 tokenId,) = _mintTo(alice, alicePk, "aave-guard.graphminis.eth", 1);
        assertEq(tokenId, 1);
        assertEq(nft.ownerOf(tokenId), alice);
        assertEq(nft.ensNameOf(tokenId), "aave-guard.graphminis.eth");
        assertEq(nft.tokenIdByEnsName(keccak256("aave-guard.graphminis.eth")), tokenId);
    }
    ...
    function test_MintRejectsProofSignedBySomeoneElse() public {
        ...
        nft.mintAgent(proofs, descs, alice, "x.graphminis.eth", "");
    }
    ...
    function test_NonceIsSingleUse() public {
        ...
        descs[0] = "manifest";
        vm.prank(alice);
        nft.mintAgent(proofs, descs, alice, "a.graphminis.eth", "");
        vm.prank(alice);
        vm.expectRevert();
        nft.mintAgent(proofs, descs, alice, "b.graphminis.eth", "");
    }

    function test_PlainErc721TransferReverts() public {
        (uint256 tokenId,) = _mintTo(alice, alicePk, "a.graphminis.eth", 1);
        vm.prank(alice);
    ...
    function test_TransferRequiresReceiverSignature() public {
        (uint256 tokenId, bytes32 oldHash) = _mintTo(alice, alicePk, "a.graphminis.eth", 1);
    ...
    function test_CloneIsAFork() public {
        (uint256 parent, bytes32 oldHash) = _mintTo(alice, alicePk, "aave-guard.graphminis.eth", 1);
        bytes[] memory proofs = new bytes[](1);
    ...
    function test_RegistryHoldsTheReverseHalfOfTheBinding() public {
        string memory ensName = "aave-guard.graphminis.eth";
        ...
        (bool bad,,) = registry.verify("someone-else.graphminis.eth", tokenId);
        assertFalse(bad);
    }
    ...
    function test_RegistryRefusesNonOwner() public {
        ...
        registry.register("a.graphminis.eth", "cid", bytes32(0), bytes32(0), tokenId, "1.0.0");
    }

    function test_UpdateFollowsTokenOwnershipNotAuthorship() public {
        string memory ensName = "a.graphminis.eth";
