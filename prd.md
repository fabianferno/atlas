...
| 2 | Reusable tooling published + installable (npm + MCP + SKILL) | Graph 1 | | **◐** MCP server live at `/api/mcp` (~~5 tools~~ **6 tools** — counted off a live `tools/list` this session: `list_schemas`, `check_coverage`, `plan_mini_app`, `query_graph`, `build_mini_app`, `resolve_mini_app`; the row was written before the sixth existed, verified handshake) and `SKILL.md` written. **npm package still not extracted** |
...
| **Trusting a library helper over the canonical source** | Med | High | viem's `getEnsResolver` returns `0x422484c2…` on Sepolia for our wrapped subnames — an address where every resolver call reverts. The ENS registry's own `resolver(node)` returns the real one. Reading the registry directly is one call and cannot be wrong; the helper was convenient and silently wrong, and the symptom was `contenthash: null` for a record that was set |
