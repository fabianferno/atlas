...
| 9 | ENS improves identity/discoverability non-cosmetically | ENS 2 | | **☑** verified from the deployed origin, no write key: `source: contenthash` (ENSIP-7, not a fallback), `addr` → the app's own wallet, both ENSIP-26 endpoints on the live origin, Agentic ID token 8, `mutuallyVerified: true`. Caveat worth knowing before the booth: `aave-health-guard` predates wallet binding and has **no `addr`** — demo `durable-market-guard` |
