 ...
 * come back `live: false` with a reason.
 *
 * WHAT HAPPENS TO A `live: false` APP has changed, and it is the reason this
 * script must read `SEED_APPS_ALL`: such an app is now DROPPED from the registry
 * rather than falling back to a hand-written fixture body. The fallback was the
 * dangerous branch — one failed resolve restored a set of plausible invented
 * numbers that sat on the board indistinguishable from the fifteen measured cards
 * beside it. Absence is honest; an unmeasured app dressed in real-looking figures
 * is not. `SEED_DROPPED` in lib/seed.ts names anything that fell out, so a
 * shorter registry is visible rather than silent.
 */
...
// a fixture body. Measuring the filtered view would make that drop permanent: one
// transient rate limit or 502 marks an app not-live, the next run no longer sees it
// to re-measure, and the registry silently shrinks by one app forever with nothing
// on screen saying so. The generator must always see everything declared.
import { SEED_APPS_ALL } from "@/lib/seed";
...
async function runOne(app: (typeof SEED_APPS_ALL)[number]): Promise<LiveSeedEntry> {
...
async function main(): Promise<void> {
  ...
  const apps = SEED_APPS_ALL.filter((a) => only.length === 0 || only.includes(a.manifest.name));
  ...
  const snapshot = {
    ...
    note:
      ...
      "An entry with `live: false` carries the reason and its app is DROPPED from " +
      "the registry — it does not fall back to a fixture body. Re-run to recover " +
      "an app that failed transiently.",
